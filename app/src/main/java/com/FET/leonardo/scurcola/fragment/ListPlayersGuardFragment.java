...
public class ListPlayersGuardFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        View v = inflater.inflate(R.layout.list_players_guard, container, false);

        final ArrayList<Player> playersNoGuard = new ArrayList<>(provider.getAlivePlayers());

        Iterator<Player> i = playersNoGuard.iterator();
        while (i.hasNext()) {
            Player player = i.next(); // must be called before you can call i.remove()
            if (player.getCard() == Card.Guard) {
                i.remove();
            }
        }
        RecyclerView myList = (RecyclerView) v.findViewById(R.id.playersNoGuard);
        myList.setLayoutManager(new LinearLayoutManager(getActivity()));
        NormalAdapter adapter = new NormalAdapter(playersNoGuard);
